	...
	void EmitTelemetry(SocketIOEvent obj)
	{
		UnityMainThreadDispatcher.Instance().Enqueue(() =>
		{
			...
			if ( !point_path.isServerProcess() ) {
				...
				JSONObject image = new JSONObject(JSONObject.Type.ARRAY);
				image.Add(Convert.ToBase64String(CameraHelper.CaptureFrame(FrontFacingCamera)));
				data["image"] = image;
		...
		// 		// localization of car
		// 		data["x"] = Car.transform.position.x.ToString("N4");
		// 		data["y"] = Car.transform.position.z.ToString("N4");
		// 		data["yaw"] = convertAngle(Car.transform.rotation.eulerAngles.y).ToString("N4");
		// 		data["speed"] = _carController.CurrentSpeed.ToString("N4");

		// 		CarAIControl carAI = (CarAIControl) Car.GetComponent(typeof(CarAIControl));
		// 		List<float> frenet_values = carAI.getThisFrenetFrame();

		// 		data["s"] = frenet_values[0].ToString("N4");
		// 		data["d"] = frenet_values[1].ToString("N4");
		...
		// 		//End path S and D values
		// 		data["end_path_s"] = end_path_s.ToString("N4");
		// 		data["end_path_d"] = end_path_d.ToString("N4");

		// 		CarTraffic cars = (CarTraffic) Car.GetComponent(typeof(CarTraffic));
		// 		data["sensor_fusion"] = JsonUtility.ToJson(cars.example_sensor_fusion());
